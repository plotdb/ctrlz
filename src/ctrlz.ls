json0 = if window? and window.json0? => window.json0 else require "json0"
...
ctrlz.prototype = Object.create(Object.prototype) <<< do
  ...
  apply: ({obj, op, src}) ->
    ...
    if !op and obj => op = json0.diff @cur, obj
  ...
  undo: ->
    ...
    for i from idx + 1 til @stack.all.length => if !@stack.all[i].src =>
      op = json0.type.transform op, @stack.all[i].op, \left
    @cur = json0.type.apply @cur, op
    opo = {op, src: true}
    @stack.all.push opo
    @stack.redo.push opo
